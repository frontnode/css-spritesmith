...
var defaultOptions = {
    ...
    useimageset: false,
    // 是否以图片文件指纹为文件名生成新的雪碧图文件，如果启用请注意清理之前生成的文件，默认不生成新文件
    newsprite: false,
    // 在CSS文件末尾追加时间戳，默认不追加
    cssstamp: false,
    // 默认使用二叉树最优排列算法
...
function replaceFilePathPlaceholder(obj, keys, placeholder, value) {
...
function cssSpriteSmith(options, callback) {
    ...
    // A big map to store all the infomation about sprite
    ...
    async.waterfall([
        ...
        function baseConfig(cb) {
            ...
            sliceData.retinaSpriteImg = fixPath(path.join(options.spritepath, cssFilename + '@2x.png'));
        ...
        function buildCss(cssData, cb) {
            ...
            replaceFilePathPlaceholder(
                ...
                '{hash}',
                getFileHash(sliceData.spriteData.image, 8)
            )
        ...
        function processResult(cssData, cb) {
            // timestamp
            if(options.cssstamp) {
                cssData += '\n.css_stamp{ content:"'+ getTimeStamp() +'";}';
            }
